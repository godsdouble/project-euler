# My logic was to reduce iterations by finding all 0 positions and avoiding those multiplications

def find(s, ch):
    return [i for i, char in enumerate(s) if char == ch]

def ignore_index(lst):
    for el in lst:
        lower = el-12
        for i in range(lower, el+1):
            ignored.add(i)

# original 1000 digit number given by Project Euler
long_string = ""

list_string = list(long_string) # needed to convert into integer list

int_list = [int(el) for el in list_string] # list that will be used in a function to determine product of 13 consecutive digits

zero_index=find(long_string,"0") # used to generate all 0 positions in given 1000 digits number

ignored=set() # this is updated by calling ignore_index function

ignore_index(zero_index) # all indices to be ignored due to multiplication by 0

try_index=set(range(0,1000)) # using this set of all numbers from 1 to 1000 and removing all indices we should ignore

try_index.difference_update(ignored) # this set contains the indices I will try to find the largest product from

biggest_product=1

for elem in try_index:
    current_product=1
    for i in range(elem, elem+13):
        current_product*=int_list[i]
    if current_product>biggest_product:
        start=elem # this is the digit position where to start multiplication for the biggest product
        biggest_product=current_product

print (start, ":", biggest_product)